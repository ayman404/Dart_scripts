...
path_data = "C:/Users/OUSSEMA/Desktop/model_Diffusion_gen/Data"

# NEW: Define reference paths as in Code 2
REFERENCE_IMAGE_PATH = r"C:\Users\OUSSEMA\Desktop\model_Diffusion_gen\Data\reference\sequence_0\sequence_0.tif"
REFERENCE_CONTEXT_PATH = r"C:\Users\OUSSEMA\Desktop\model_Diffusion_gen\Data\reference\sequence_0\context.pt"

# Add downsample_MTF and related functions
...
def downsample_MTF(img, val_pos):
    ...
    xr = 1
    xc = 1
    return pan2m_Blurred[(int((hsize+1)/2+xr-1)):(int((hsize+1)/2+row-1+xr)):4, 
                         int((hsize+1)/2+xc-1):int(((hsize+1)/2+col-1+xc)):4]
...
def visualize_forward_process(config, diffusion, epoch):
    with rasterio.open(path_data + "/train/sequence_0/sequence_0.tif") as src:
        ...
        downsampled_img = torch.zeros((img.shape[0], img.shape[1]//4, img.shape[2]//4))
        for i in range(img.shape[0]):
            downsampled_img[i] = torch.tensor(downsample_MTF(img[i].numpy(), 0.15))
        img = downsampled_img.to(device)
    ...
    for i, t in enumerate(timesteps):
        noised_img, _, _ = diffusion.forward_diffusion(
            img.unsqueeze(0),
...
def analyze_data_structure(data_root):
    try:
        ...
        with rasterio.open(tif_path) as src:
            ...
            if len(img_data.shape) == 2:
                img_data = np.expand_dims(img_data, axis=0)
            downsampled_shape = downsample_MTF(img_data[0], 0.15).shape
            num_bands, height, width = src.count, downsampled_shape[0], downsampled_shape[1]

        context_path = sample_seq / "context.pt"
        if not context_path.exists():
            raise FileNotFoundError(f"Context file not found: {context_path}")

        context_sample = torch.load(context_path, map_location="cpu", weights_only=False)
...
class DynamicConfig:
    def __init__(self, data_specs):
        ...
        self.output_dir = self.data_root / "MTF"
        ...
        print(f"Alpha bar at t=1000: {self.alpha_bar[999].item():.5f}")
...
class DiffusionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            with rasterio.open(self.train_dir / seq / f"{seq}.tif") as src:
                ...
                if len(img_data.shape) == 2:
                    img_data = np.expand_dims(img_data, axis=0)
                downsampled_img = np.zeros((img_data.shape[0], img_data.shape[1]//4, img_data.shape[2]//4))
                for i in range(img_data.shape[0]):
                    downsampled_img[i] = downsample_MTF(img_data[i].astype(np.float32), 0.15)
                img = torch.tensor(downsampled_img).float()

            with rasterio.open(REFERENCE_IMAGE_PATH) as src_ref:
                img_ref_data = src_ref.read()
                if len(img_ref_data.shape) == 2:
                    img_ref_data = np.expand_dims(img_ref_data, axis=0)
                downsampled_ref = np.zeros((img_ref_data.shape[0], img_ref_data.shape[1]//4, img_ref_data.shape[2]//4))
                for i in range(img_ref_data.shape[0]):
                    downsampled_ref[i] = downsample_MTF(img_ref_data[i].astype(np.float32), 0.15)
                img_ref = torch.tensor(downsampled_ref).float()

            if self.normalize_before_diff:
                img = (img - self.config.image_mean.cpu().view(-1, 1, 1)) / self.config.image_std.cpu().view(-1, 1, 1)
                img_ref = (img_ref - self.config.image_mean.cpu().view(-1, 1, 1)) / self.config.image_std.cpu().view(-1, 1, 1)
            ...
            context = torch.load(self.train_dir / seq / "context.pt", map_location="cpu", weights_only=False)
            context_ref = torch.load(REFERENCE_CONTEXT_PATH, map_location="cpu", weights_only=False)

            if len(context.shape) == 2:
                context = context.unsqueeze(0)
            if len(context_ref.shape) == 2:
                context_ref = context_ref.unsqueeze(0)

            context_diff = context - context_ref
    ...
    def _compute_statistics(self):
        ...
        self.image_std = torch.clamp(self.image_std, min=1e-6)
        self.context_std = torch.clamp(self.context_std, min=1e-6)
...
def create_diffusion_model(config):
    ...
    class DynamicUNet(nn.Module):
        def __init__(self):
            ...
            self.context_channels = config.context_dims[0]
            ...
            for i in range(self.num_downsample):
                h = config.padded_dims[0] // (2 ** (self.num_downsample - i - 1))
                w = config.padded_dims[1] // (2 ** (self.num_downsample - i - 1))
                self.spatial_shapes.append((h, w))
            ...
            for i in range(self.num_downsample):
                ...
                up_in_ch = current_ch
                up_out_ch = up_in_ch // 2
                concat_ch = up_out_ch + skip_ch

                use_context = i < self.num_downsample  #3  Inject context in up1, up2, up3
                if use_context:
                    target_shape = self.spatial_shapes[i]
                    self.context_encoders.append(nn.Sequential(
                        nn.Conv2d(self.context_channels, up_out_ch, 3, padding=1), #garde les dimensions du contexte original
                        nn.GELU(),
                        nn.AdaptiveAvgPool2d(target_shape)
                    ))
                else:
                    self.context_encoders.append(nn.Identity())

                self.up_blocks.append(nn.Sequential(
                    nn.Sequential(
                        nn.Upsample(scale_factor=2, mode='nearest'),
                        nn.Conv2d(up_in_ch, up_out_ch, 3, padding=1)
                    ),
                    DynamicBlock(concat_ch, up_out_ch, self.time_dim)
                ))
                current_ch = up_out_ch
        ...
        def forward(self, x, t, context):
            ...
            for idx, block in enumerate(self.up_blocks):
                ...
                context_input = self.context_encoders[idx](context)

                if not isinstance(self.context_encoders[idx], nn.Identity):
                    if context_input.shape != x.shape:
                        raise RuntimeError(f"Shape mismatch: x={x.shape}, context_input={context_input.shape}")
                    x = x * context_input  # Apply context multiplicatively
            ...
            noise_pred, x0_pred = torch.chunk(output, 2, dim=1)
...
class DiffusionProcess(torch.nn.Module):
    ...
    def reverse_diffusion_step(self, model, xt, t, context, use_ddim=True):
        ...
        with torch.no_grad():
            ...
            pred_x0 = 0.5 * pred_x0_direct + 0.5 * pred_x0_from_noise
    ...
    try:
        for epoch in range(config.epochs):
            ...
            for x0, context in tqdm(train_loader, desc=f"Epoch {epoch+1}/{config.epochs}"):
                ...
                xt, noise, clean_x0 = diffusion.forward_diffusion(x0, t)
                ...
                with torch.amp.autocast('cuda'):
                    ...
                    for band_idx in range(pred_noise.shape[1]):
                        ...
                        pred_band_up = F.interpolate(pred_band, size=(224, 224), mode='bilinear', align_corners=False)
                        noise_band_up = F.interpolate(noise_band, size=(224, 224), mode='bilinear', align_corners=False)
                        pred_band_norm = 2 * (pred_band_up - pred_band_up.min()) / (pred_band_up.max() - pred_band_up.min() + 1e-8) - 1
                        noise_band_norm = 2 * (noise_band_up - noise_band_up.min()) / (noise_band_up.max() - noise_band_up.min() + 1e-8) - 1
                    ...
                    for band_idx in range(pred_x0_direct.shape[1]):
                        ...
                        pred_band_up = F.interpolate(pred_band, size=(224, 224), mode='bilinear', align_corners=False)
                        x0_band_up = F.interpolate(x0_band, size=(224, 224), mode='bilinear', align_corners=False)
                        pred_band_norm = 2 * (pred_band_up - pred_band_up.min()) / (pred_band_up.max() - pred_band_up.min() + 1e-8) - 1
                        x0_band_norm = 2 * (x0_band_up - x0_band_up.min()) / (x0_band_up.max() - x0_band_up.min() + 1e-8) - 1
                        pred_band_norm = pred_band_norm.repeat(1, 3, 1, 1)
            ...
            with torch.no_grad():
                for x0, context in val_loader:
                    ...
                    pred_noise, _ = model(xt, t, context)
                    mse = F.mse_loss(pred_noise, noise)
                    psnr = 10 * torch.log10(1 / mse)
                    val_psnr += psnr.item()
                    val_loss += mse.item()
            ...
            if (epoch + 1) % 3 == 0:
                ...
                with torch.no_grad():
                    for x0, context in val_loader:
                        x0 = x0.to(device)
                        context = context.to(device)
                        x0_single = x0[0:1]  # Shape: (1, 10, 20, 16) due to padding
                        context_single = context[0:1]  # Shape: (1, 3, 20, 16)

                        sqrt_alpha_bar = torch.sqrt(config.alpha_bar[999]).view(-1, 1, 1, 1)
                        sqrt_one_minus_alpha_bar = torch.sqrt(1 - config.alpha_bar[999]).view(-1, 1, 1, 1)
                        noise = torch.randn_like(x0_single)  # Shape: (1, 10, 20, 16)
                        xt = sqrt_alpha_bar * x0_single + sqrt_one_minus_alpha_bar * noise

                        pred_noise, pred_x0_direct = model(xt, torch.tensor([999], device=device).repeat(1), context_single)
                        alpha_bar_t = config.alpha_bar[999].view(-1, 1, 1, 1)
                        ...
                        pred_x0_original = pred_x0_original[:, :, :config.img_dims[0], :config.img_dims[1]]  # Shape: (1, 10, 20, 12)
                        ...
                        with rasterio.open(REFERENCE_IMAGE_PATH) as src_ref:
                            ...
                            for i in range(img_ref_data.shape[0]):
                                downsampled_ref[i] = downsample_MTF(img_ref_data[i].astype(np.float32), 0.15)
                            img_ref = torch.tensor(downsampled_ref).float().to(device)  # Shape: (10, 20, 12)

                        # Debug shapes
                        print(f"pred_x0_original shape: {pred_x0_original.shape}")
                        print(f"img_ref shape: {img_ref.shape}")
                        print(f"noise shape before cropping: {noise.shape}")

                        # Add reference image
                        pred_x0_original = pred_x0_original + img_ref.unsqueeze(0)  # Both (1, 10, 20, 12)

                        # Crop noise to original dimensions
                        noise = noise[:, :, :config.img_dims[0], :config.img_dims[1]]  # Shape: (1, 10, 20, 12)

                        generated = generate_samples(model, config, num_samples=1, context=context_single)
                        generated = generated[:, :, :config.img_dims[0], :config.img_dims[1]]  # Shape: (1, 10, 20, 12)
                        ...
                        for band_idx in range(generated.shape[1]):
                            ...
                            orig_band = x0_single[0, band_idx, :config.img_dims[0], :config.img_dims[1]].cpu().numpy()
                            axs[0].imshow(orig_band, cmap='gray')
                            axs[0].set_title(f"Original Band {band_idx+1}")
                            axs[0].axis('off')

                            noise_band = noise[0, band_idx].cpu().numpy()
                            axs[1].imshow(noise_band, cmap='gray')
...
def generate_samples(model, config, num_samples=1, context=None, num_steps=None, use_ddim=True):
    ...
    for t in tqdm(reversed(step_sequence), desc="Generating", total=len(step_sequence)):
        pred_noise, pred_x0_direct = model(x_t, torch.full((num_samples,), t, device=device, dtype=torch.long), context)
        x_t = diffusion.reverse_diffusion_step(
            model, x_t, t, context, use_ddim=use_ddim
        )

    generated_diff = x_t[:, :, :config.img_dims[0], :config.img_dims[1]]  # Crop to (20, 12)
    ...
    generated = generated_diff + img_ref.unsqueeze(0)  # Both (num_samples, 10, 20, 12)
...
def test_model(config, model_path, context_path, num_samples=1):
    try:
        ...
        target_context = torch.load(context_path, map_location=device, weights_only=False)
        context_ref = torch.load(REFERENCE_CONTEXT_PATH, map_location=device, weights_only=False)

        context_diff = target_context - context_ref
